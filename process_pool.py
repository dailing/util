...
class CachedFunction(object):
    ...
    @staticmethod
    def hash_key(key, hash_obj):
        if type(key) is bytes:
            hash_obj.update(key)
        elif type(key) is str:
            hash_obj.update(bytes(key, encoding='utf-8'))
        else:
            hash_obj.update(pickle.dumps(key))
        return
    ...
    def default_key_func(*args, **kwargs):
        md5 = hashlib.md5()
        for i in args:
            CachedFunction.hash_key(i, md5)
        for k, v in kwargs.items():
            CachedFunction.hash_key(k, md5)
            CachedFunction.hash_key(v, md5)
        return md5.hexdigest()

    def __init__(
            self,
            cache_dir='runs/cache',
            key_func=default_key_func.__func__):
        self.func = None
        self.cache_dir = cache_dir
        self.key_func = key_func
        logger.info(key_func)
        self.source_code_md5 = None
    ...
    def __call__(self, *args, **kwargs):
        if self.func is None:
            ...
            md5 = hashlib.md5()
            md5.update(bytes(source_code, encoding='utf-8'))
            self.source_code_md5 = md5.hexdigest()
            logger.info(source_code)
            logger.info(self.source_code_md5)
            return self
        else:
            key = self.key_func(*args, **kwargs)
            file_path = pjoin(self.cache_dir, f'{self.source_code_md5}.{key}')
            if os.path.exists(file_path):
                logger.debug(f'getting {file_path}')
